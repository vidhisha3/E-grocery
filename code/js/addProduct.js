...
//checking the users is logged in or not
window.onload = () => {
    if(user){
...
var round =Math.round(actualPrice);
var round =Math.round(sellingPrice);
//upload image handle
...
const validateForm = () => {
    if(!productName.value.length){
       return showAlert('enter the product name');
    }else if(shortLine.value.length > 100 || shortLine.value.length < 10 ){
        return showAlert('short description must to be 10 to 100 letters long');
    }else if(!des.value.length){
        return showAlert ('enter detail description');
    }else if(!imagePaths.length){ //imagelink array
        ...
        return showAlert('you should have at least 20 in stock');
    }else if(!tags.value.length){
        return showAlert('enter few to help ranking you product in search');
    }else if(!tac.checked){
...
const fetchProductData = () => {
    //delete the rempProduct from the session
    fetch('/get-products',{
        method:'Post',
